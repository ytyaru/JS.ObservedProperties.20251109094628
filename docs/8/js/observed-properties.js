...
class DecimalNumber extends Quantity {// 十進数における整数と少数を合わせて15桁までの有限数(IEEE754において整数部は15桁までが正確に表現できる上限)
...
//        if (DecimalNumber.MAX_FIG < fig) {throw new RangeError(`figは15以下であるべきです。:${fig}`)}
        this.#validFig(fig);
        this._.fig = {i:DecimalNumber.MAX_FIG-fig, f:f} // 整数部/少数部の桁数
        ...
        this._.S = null;// 文字列型としての表現;
        this._.N = null;// Number型としての実数;
        this.#roundedFloat(value, fig);
    }
    #validFig(fig) {if(!(Number.isSafeInteger(fig) && DecimalNumber.MIN_FIG<=fig && fig<=DecimalNumber.MAX_FIG)){throw new TypeError(`figは0〜15の整数値であるべきです。:${fig}`)}}
...
//        this.#validFig(o.fig.i); this.#validFig(o.fig.f);
        return `${this._.I.toString().padStart(o.fig.i, o.pad.i)}.${this._.F.toString().padStart(o.fig.f, o.pad.f)}`;
    }
    /*
...
//        this.#setupMinMax(min, max)
        //this._ = {...Integer.validate(value, unsafed, unsigned, bit, min, max), ...Integer.validateMinMax(unsafed, unsigned, bit, min, max)};
        this._ = {...Integer.validate(value, unsafed, unsigned, bit, min, max)};
    }
}
class FuzzyInteger extends Quantity {
}
