...
const prng32MinMax = (min, max) => {
    const r  = crypto.getRandomValues(new Uint32Array(1))[0] / 4294967295; // Math.random()
...
const _csprngMinMaxCheck = (min, max)=>{// min,maxが32bit範囲内で大小関係を保った正数か確認する
    //if (Number.isSafeInteger(min) && Number.isSafeInteger(max) && 0<=min && max<2**32) {throw new TypeError(`minとmaxはNumber型整数値であるべきです。minは0以上max以下、maxは2**32(4294967296)未満であるべきです。:min:${min}, max:${max}`)}
    if (Number.isSafeInteger(min) && Number.isSafeInteger(max)) {throw new TypeError(`minとmaxはNumber型整数値であるべきです。minは0以上max以下、maxは2**32(4294967296)未満であるべきです。:min:${min}, max:${max}`)}
    if (min < 0) {throw new RangeError(`minは0以上のNumber型整数値であるべきです。:min:${min}`)}
    if (4294967296 <= max) {throw new RangeError(`maxは0以上のNumber型整数値であるべきです。:max:${max}`)}
    if (max <= min) {throw new RangeError(`minとmaxは1以上の差があるべきです。:min:${min}, max:${max}`)}
}
const _csprngMinMaxRnd = (min, max)=>{return max < 256 ? new Uint8Array(1) : (max < 65536 ? new Uint16Array(1) : new Uint32Array(1))}// 32bit範囲内で均等分布の乱数を返す
/*
const csprngMinMaxRnd(min, max) {// 32bit範囲内で均等分布の乱数を返す
    return max < 256 ? new Uint8Array(1) : (max < 65536 ? new Uint16Array(1) : new Uint32Array(1));
//    if (max < 256) {return new Uint8Array(1)}
//    else if (max < 65536) {return new Uint16Array(1)}
//    else if (max < 4294967296) {return new Uint32Array(1)}
//    else {}// ここは例外発生して通らないはず
}
*/
// 32bitまでの間にある指定した開始〜終了までの範囲の数において、偏りが出ないよう均等分布させた乱数を返す。
// 例えばサイコロ1〜6を出すとき2^8=256(0〜255)の乱数から算出するが、256 % 6 = 4 となり0でない場合は偏りが出る。よってrnd % 6では偏るため他の方法で算出する。
// すなわち範囲内の数が出るまで繰り返す。サイコロなら0〜255の乱数のうち1〜6の値が出るまで乱数を引き続ける。これにて偏りが無くなり均等分布になる。
const csprng32MinMax = (min, max)=>{
    _csprngMinMaxCheck(min, max);
    const b = _csprngMinMaxRnd(min, max); // 8,16,32bitのいずれかのバイナリ配列を取得する
    let r = -1;
    do {r = crypto.getRandomValues(b)[0];} while(r > max || r < min); // 暗号論的疑似乱数を生成する。範囲内値が出現するまで繰り返す。
//    while (r > max || r < min) {r = crypto.getRandomValues(b)[0];} // 暗号論的疑似乱数を生成する。範囲内値が出現するまで繰り返す。
    return r;
}
const _calcMask = (range)=>{
...
const csprngNumMinMax = (min, max) => {// Number型範囲内なら正数負数問わず乱数を返す(Number.SAFE_MIN_INTEGER〜Number.SAFE_MAX_INTEGER)
    ...
    const range = max - min;
    const {bitsNeeded, bytesNeeded, mask} = _calcMask(range);
    /*
    if (range < -9007199254740991 || range > 9007199254740991) {throw new RangeError(`minとmaxの差は±9007199254740991以内であるべきです。`)}
    let bitsNeeded = 0;
    let bytesNeeded = 0;
    ...
    */
    ...
    r = r & mask;
    return r <= range ? min+r : prngNumMinMax(min, max);
}
