...
class DecimalNumber extends Float {// 十進数における整数と少数を合わせて15桁までの有限数(IEEE754において整数部は15桁までが正確に表現できる上限)
    static validate(value, fig, unsigned=false, min=undefined, max=undefined) {
        this.validFig(fig);
//        return Quantity.validate(value, false, false, false, unsigned, min, max);
    ...
    constructor(value, fig, unsigned=false, min=undefined, max=undefined) {
        ...
        this.validFig(fig);
        this._.fig = fig;
//        this._ = DecimalNumber.validate(value, fig, unsigned, min, max);
    }
    validate(v) {return DecimalNumber.validate(v ?? this.value, this._.fig, this._.unsigned, this._.min, this._.max)}
    get fig() {return this._.fig} // 少数部の桁数
    get intFig() {return DecimalNumber.MAX_FIG - this._.fig} // 整数部の桁数
    ...
    set partI(v) {// 整数部に代入する
        ...
        this.value = Math.trunc(v) + this.partF;
    }
    set partF(v) {// 少数部に代入する（値はNumber型の整数）少数で代入したい場合はvalueで整数部を含めてする。少数部だけで代入する時は正確に指定できる整数で代入する。
        ...
        this.value = this.partI + (v / ((10)**this._.fig));
    }
...
//    get step() {return new DecimalNumber(0===this._.fig.f ? '0' : `0.${'0'.repeat(this._.fig.f-1)}1`, this._.fig.f);}
    ...
    //get maxFlt() {return 0===this._.fig ? 0 : parseInt('0.'+'9'.repeat(this.fig))} // 少数部の最小値を少数で返す
    //get maxFlt() {return 0===this._.fig ? 0 : (this.maxFltI - (this.partI * Number.EPSILON)) / this.maxFltI} // 少数部の最小値を少数で返す
...
//        if (this._.fig < FIG) {return throw new RangeError(`figが範囲外です。${DecimalNumber.MIN_FIG}〜${this._.fig} までの整数値であるべきです。`)}
        if (!(Number.isSafeInteger(fig) && 0<=fig && fig<=this._.fig)) {throw new TypeError(`figは ${DecimalNumber.MIN_FIG}〜${this._.fig} の整数であるべきです。`)}
